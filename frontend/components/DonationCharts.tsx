...
import { DonationAnalytics } from '../services/donations';
...
export default function DonationCharts({ analytics }: DonationChartsProps) {
    const [selectedChart, setSelectedChart] = useState<'monthly' | 'yearly'>('monthly');

    const maxMonthlyCount = Math.max(...analytics.monthlyStats.map(s => s.count), 1);
    const maxYearlyCount = Math.max(...analytics.yearlyStats.map(s => s.count), 1);
    ...
    const renderMonthlyChart = () => {
        ...
        return (
            <View style={styles.chartContainer}>
                <Text style={styles.chartTitle}>Doações por Mês (Últimos 12 meses)</Text>

                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                    <View style={styles.chart}>
                        {/* Y-axis labels */}
                        <View style={styles.yAxis}>
                            {[maxMonthlyCount, Math.floor(maxMonthlyCount * 0.75), Math.floor(maxMonthlyCount * 0.5), Math.floor(maxMonthlyCount * 0.25), 0].map((value, index) => (
                                <View key={index} style={[styles.yAxisLabel, { top: (index * maxBarHeight / 4) - 8 }]}>
                                        ...
                                        <View style={styles.barWrapper}>
                                            <View
                                                style={[
                                                    ...
                                                    {
                                                        height: barHeight || 2,
                                                        backgroundColor: stat.count > 0 ? '#E73645' : '#E5E7EB',
                                                    }
                                                ]}
                                            />
                                            {stat.count > 0 && (
                ...
                <View style={styles.monthlyStats}>
                    ...
                    <View style={styles.statItem}>
                        <Text style={styles.statValue}>
                            {Math.round(analytics.monthlyStats.reduce((sum, s) => sum + s.count, 0) / 12 * 10) / 10}
                        </Text>
    ...
    const renderYearlyChart = () => {
        const barWidth = Math.min(80, (chartWidth - 60) / analytics.yearlyStats.length);
        const maxBarHeight = chartHeight - 60;
        ...
        return (
            <View style={styles.chartContainer}>
                ...
                <View style={styles.chart}>
                    {/* Y-axis labels */}
                    <View style={styles.yAxis}>
                        {[maxYearlyCount, Math.floor(maxYearlyCount * 0.75), Math.floor(maxYearlyCount * 0.5), Math.floor(maxYearlyCount * 0.25), 0].map((value, index) => (
                            <View key={index} style={[styles.yAxisLabel, { top: (index * maxBarHeight / 4) - 8 }]}>
                                <Text style={styles.axisText}>{value}</Text>
                            </View>
                        ))}
                ...
                {analytics.yearlyStats.length > 1 && (
                    <View style={styles.yearlyComparison}>
                        <View style={styles.comparisonItem}>
                            <Ionicons
                                name={analytics.donationsThisYear >= analytics.donationsLastYear ? "trending-up" : "trending-down"}
                                size={16}
                                color={analytics.donationsThisYear >= analytics.donationsLastYear ? "#10B981" : "#F59E0B"}
                            />
            ...
            <View style={styles.progressContainer}>
                ...
                <View style={styles.progressRing}>
                    ...
                    <View style={styles.progressContent}>
                        <Text style={styles.progressNumber}>{analytics.donationsThisYear}</Text>
            ...
            <View style={styles.overallStats}>
                ...
                <View style={styles.statsGrid}>
                    ...
                    <View style={styles.statCard}>
                        ...
                        <Text style={styles.statCardValue}>
                            {analytics.lastDonationDate ?
                                new Date(analytics.lastDonationDate).toLocaleDateString('pt-BR', {
                                    day: '2-digit',
                                    month: '2-digit',
                                    year: '2-digit'
                                }) :
                    ...
                    <View style={styles.statCard}>
                        ...
                        <Text style={styles.statCardValue}>
                            {Math.round(analytics.totalVolume / 450)} {/* Approximate lives saved */}
                        </Text>
