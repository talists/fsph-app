...
import { getDoadorInfoByCPF, getAgendamentosByCPF } from './hemose';
...
class DonationsService {
    ...
    async fetchDonationHistory(cpf: string): Promise<DonationRecord[]> {
        try {
            ...
            if (agendamentos && Array.isArray(agendamentos)) {
                agendamentos.forEach((agendamento: any, index: number) => {
                    const donation: DonationRecord = {
                        ...
                        date: this.formatDate(agendamento.data_agendamento || agendamento.date),
            ...
            console.warn('⚠️ Failed to fetch from HEMOSE API, using cached data:', error);
    ...
    private async getCachedDonations(cpf: string): Promise<DonationRecord[]> {
        try {
            const cached = await AsyncStorage.getItem(`${this.storageKeys.donations}_${cpf}`);
            if (cached) {
     ...
     */
    private async cacheDonations(cpf: string, donations: DonationRecord[]): Promise<void> {
        try {
    ...
    calculateAnalytics(donations: DonationRecord[]): DonationAnalytics {
        ...
        return {
            ...
            donationsThisYear: completedDonations.filter(d =>
                new Date(d.date).getFullYear() === currentYear
            ).length,
            donationsLastYear: completedDonations.filter(d =>
     ...
     */
    async scheduleNextDonationReminder(userId: string, nextEligibleDate: Date, gender: 'M' | 'F'): Promise<void> {
        try {
            ...
            if (reminderDate > new Date()) {
                const notificationId = await Notifications.scheduleNotificationAsync({
                    content: {
                        ...
                        body: `Você poderá doar sangue em ${this.formatDateBR(nextEligibleDate)}! Que tal agendar?`,
                        data: {
            ...
            if (nextEligibleDate > new Date()) {
                const eligibilityId = await Notifications.scheduleNotificationAsync({
                    ...
                    trigger: {
                        type: Notifications.SchedulableTriggerInputTypes.TIME_INTERVAL,
                        seconds: Math.max(1, Math.floor((nextEligibleDate.getTime() - Date.now()) / 1000))
                    },
                });
     ...
     */
    async scheduleBloodStockAlert(userId: string, bloodType: string): Promise<void> {
        try {
            const notificationId = await Notifications.scheduleNotificationAsync({
                content: {
    ...
    private mapStatus(status: string): 'completed' | 'pending' | 'cancelled' {
        ...
        if (statusLower.includes('concluí') || statusLower.includes('realiz')) return 'completed';
        if (statusLower.includes('cancel') || statusLower.includes('rejeit')) return 'cancelled';
        return 'pending';
    }

    private mapDonationType(tipo: string): 'individual' | 'campaign' | 'bone_marrow' {
        switch (tipo) {
            case 'M': return 'bone_marrow';
            case 'C': return 'campaign';
    ...
    private async storeNotificationSchedule(schedule: NotificationSchedule): Promise<void> {
        try {
    ...
    private async getScheduledNotifications(userId: string): Promise<NotificationSchedule[]> {
        try {
            const stored = await AsyncStorage.getItem(`${this.storageKeys.notifications}_${userId}`);
    ...
    private async saveScheduledNotifications(userId: string, notifications: NotificationSchedule[]): Promise<void> {
        try {
            await AsyncStorage.setItem(
