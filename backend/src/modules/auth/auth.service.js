...
export class AuthService {
  static async login(email, senha) {
    ...
    if (!usuario.senha)
      throw new Error(
        "Conta vinculada com a Google. Use login com o provedor."
      );
    ...
    const refreshToken = jwt.sign({ id: usuario.id }, JWT_REFRESH_SECRET, {
  ...
  static async refreshToken(tokenRecebido) {
    ...
    const payload = jwt.verify(tokenRecebido, JWT_REFRESH_SECRET);
    ...
    await redisClient.set(`token_usuario_${usuario.id}`, newAccessToken, {
      EX: 60 * 15,
    });
  ...
  static async esqueciSenha(usuarioEmail) {
    ...
    if (!usuario) {
      return {
        msg: "Se um usuário com este e-mail existir, um link de redefinição foi enviado.",
      };
    }
