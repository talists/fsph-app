...
import donationsService, {
  DonationRecord as ServiceDonationRecord,
  DonationAnalytics,
...
export default function DonationHistory({
  ...
  const [selectedPeriod, setSelectedPeriod] = useState<"12months" | "3years">(
    "12months"
  );
  ...
  const loadDonations = async () => {
    ...
    try {
      ...
      Alert.alert(
        ...
        [{ text: "OK" }, { text: "Tentar Novamente", onPress: loadDonations }]
      );
    } finally {
      setLoading(false);
    }
  ...
  const getTotalVolume = () =>
    donations.reduce((total, donation) => total + donation.volume, 0);
  ...
  const generateDonationCertificate = (donation: ServiceDonationRecord) => {
    Alert.alert(
      "Atestado de Doação",
      `Gerar atestado para a doação realizada em ${formatDate(
        donation.date
      )} no ${donation.location}?`,
      [
